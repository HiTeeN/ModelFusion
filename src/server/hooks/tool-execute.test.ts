...
function makeAfterInput(overrides: Partial<ToolExecuteAfterInput> = {}): ToolExecuteAfterInput {
  return {
    tool: "fusion:deliberate",
    sessionID: "test-session",
    callID: "call-1",
...
function makeAfterOutput(overrides: Partial<ToolExecuteAfterOutput> = {}): ToolExecuteAfterOutput {
  return {
    title: "",
    output: "",
    metadata: {} as any,
...
describe("createToolExecuteBeforeHook", () => {
  test("fusion:deliberate — validates args, checks recursion, marks active", async () => {
    ...
    const state: ToolExecutePluginState = { recursionGuard: guard };
  ...
  test("fusion:deliberate — blocks nested fusion when already active", async () => {
    ...
    await expect(hook(input, output)).rejects.toThrow(
      "Fusion already running in this session",
    );
  });
  ...
  test("fusion:deliberate — rejects missing prompt arg", async () => {
    ...
    await expect(hook(input, output)).rejects.toThrow(
      "fusion:deliberate requires a non-empty 'prompt' argument",
    );
  });
...
describe("createToolExecuteAfterHook", () => {
  test("fusion:deliberate — formats output with analysis, answer, cost, marks complete", async () => {
    ...
    const state: ToolExecutePluginState = { recursionGuard: guard, fusionResult };
