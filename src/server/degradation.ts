...
export function handleDegradation(
  ...
  const hasSuccessfulResponse = result.responses.some(
    (r) => r.error === undefined,
  );
  const allFailed =
  ...
  if (allFailed) {
    return {
      ...
      synthesizedAnswer:
        originalModelResponse ?? result.synthesizedAnswer,
    };
  }
 ...
 */
export function getDegradationMessage(result: FusionResult): string {
  if (result.status === "ok") {
    return "";
  }
