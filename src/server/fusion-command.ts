...
export function parseFusionCommand(
  command: string,
  args: string,
): FusionCommandIntent | null {
...
export function parseFusionPromptText(text: string): FusionCommandIntent | null {
...
export function formatFusionConfigForDisplay(config: FusionConfig): string {
...
export function fusionResultToParts(result: FusionResult): Part[] {
  ...
  if (result.status === "degraded" && result.responses.length > 0) {
    ...
    return [
      textPart(
        "⚠️ Fusion panel completed but judge analysis failed. Raw panel responses below:\n\n"
          + rawResponses,
      ),
    ];
  }
...
export function invalidCommandMessageToParts(message: string): Part[] {
