...
// ===========================================================================
import { runFusionPipeline } from "./server/pipeline.js";
import { createChatMessageHook } from "./server/hooks/chat-message.js";
// ===========================================================================
// Shared fixtures
// ===========================================================================
...
function successPanelResult(providerId, modelId, content) {
    return {
        providerId,
        modelId,
        content,
...
function errorPanelResult(providerId, modelId, error) {
    return {
        providerId,
        modelId,
        content: "",
...
describe("ModelFusion Integration", () => {
    ...
    describe("runFusionPipeline", () => {
        ...
        test("GIVEN all sub-modules succeed WHEN pipeline runs THEN status ok with analysis and synthesizedAnswer", async () => {
            ...
            const result = await runFusionPipeline(dummyClient, SESSION_ID, TEST_PROMPT, defaultConfig, originalModel, guard);
            // THEN status is "ok"
        ...
        test("GIVEN happy path pipeline WHEN pipeline executes THEN cost tracker methods are called for each stage", async () => {
            ...
            const guard = freshGuard();
            // WHEN runFusionPipeline is called
            await runFusionPipeline(dummyClient, SESSION_ID, TEST_PROMPT, defaultConfig, originalModel, guard);
    ...
    describe("chat.message hook integration", () => {
        ...
        test("GIVEN triggering=auto WHEN message arrives THEN pipeline is called and output is replaced with synthesized answer", async () => {
            ...
            const pipelineCall = pipelineMock.mock.calls[0];
    ...
    describe("/fusion command integration", () => {
        ...
        test("GIVEN /fusion keymap command with confirmed question WHEN run() fires THEN toast shown and pipeline delegated", async () => {
            ...
            expect(infoToast[0].title).toBe("Fusion");
            expect(infoToast[0].message).toContain("Fan-out started");
