 ...
 */
export function createFusionCommand(api) {
    return {
        title: "Fusion: Deliberate",
        value: "fusion:deliberate",
        description: "Multi-model deliberation: panel of models → judge → structured analysis. " +
        ...
        onSelect: async () => {
            ...
            if (!question || !question.trim()) {
                api.ui.toast({
                    variant: "warning",
                    title: "Fusion",
                    message: "No question provided. Type your question after /fusion, " +
...
async function extractQuestion(api) {
    ...
    return new Promise((resolve) => {
        dialog.replace(() => api.ui.DialogPrompt({
            ...
            description: () => "Enter your question for multi-model analysis. " +
                "A panel of AI models will deliberate, a judge will compare " +
                "their responses, and a synthesizer will produce a final answer.",
            placeholder: "e.g., What are the trade-offs between monoliths and microservices?",
            onConfirm: (value) => {
                dialog.clear();
                resolve(value);
            },
