...
function buildAnalysisSummary(result) {
    ...
    const lines = ["## Fusion Panel Analysis"];
    // Consensus points
    ...
    if (analysis.scoring.length > 0) {
        ...
        for (const s of analysis.scoring) {
            lines.push(`- **${s.modelId}**: total ${s.total}/40 ` +
 ...
 */
function buildMessage(role, text, result) {
    const base = {
 ...
 *   unchanged.
 */
export function createMessagesTransformHook(pluginState) {
    return async (_input, output) => {
        ...
        const answerText = fusionResult.synthesizedAnswer ??
            "Fusion completed but no synthesized answer was produced.";
        // Inject: preserve original messages, append analysis + answer
        output.messages.push(buildMessage("system", analysisText, fusionResult), buildMessage("assistant", answerText, fusionResult));
    };
}
