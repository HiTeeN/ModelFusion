...
// Fixtures
// ---------------------------------------------------------------------------
function okFusionResult(overrides = {}) {
    return {
        status: "ok",
        analysis: {
            consensus: [
        ...
        synthesizedAnswer: "Based on panel analysis, the sky is blue. Claude noted atmospheric scattering, " +
...
function userMessage() {
    return {
        ...
        parts: [{ type: "text", text: "What color is the sky?" }],
    };
}
// ---------------------------------------------------------------------------
...
describe("createMessagesTransformHook", () => {
    ...
    test("injects analysis and answer when fusionResult is present", async () => {
        ...
        expect(analysisMsg.info.role).toBe("system");
        expect(analysisMsg.parts).toHaveLength(1);
        expect(analysisMsg.parts[0].text).toContain("Fusion Panel Analysis");
        expect(analysisMsg.parts[0].text).toContain("Consensus Points");
        expect(analysisMsg.parts[0].text).toContain("The sky is blue");
        ...
        expect(answerMsg.parts[0].text).toBe(okFusionResult().synthesizedAnswer);
    });
    // -----------------------------------------------------------------------
    ...
    test("handles missing analysis gracefully", async () => {
        ...
        await hook({}, output);
        expect(output.messages).toHaveLength(3);
        const analysisText = output.messages[1].parts[0].text;
    ...
    test("handles missing synthesized answer gracefully", async () => {
        ...
        await hook({}, output);
        expect(output.messages).toHaveLength(3);
        const answerText = output.messages[2].parts[0].text;
        expect(answerText).toContain("no synthesized answer");
    });
});
//# sourceMappingURL=messages-transform.test.js.map