...
export function createToolExecuteBeforeHook(pluginState) {
    return async (input, output) => {
        ...
        if (pluginState.recursionGuard.isFusionActive(input.sessionID)) {
            throw new Error("Fusion already running in this session — nested deliberation blocked");
        }
        // Mark fusion active
        const activated = pluginState.recursionGuard.markFusionActive(input.sessionID);
        if (!activated) {
            throw new Error("Failed to activate fusion — session may already have an active fusion call");
        }
    };
}
// ---------------------------------------------------------------------------
...
export function createToolExecuteAfterHook(pluginState) {
    return async (input, output) => {
        ...
        // --- Cost ---
        sections.push("## Cost");
        sections.push(`- Prompt tokens: ${result.cost.totalPromptTokens.toLocaleString()}`);
        sections.push(`- Completion tokens: ${result.cost.totalCompletionTokens.toLocaleString()}`);
        sections.push(`- Estimated cost: $${result.cost.estimatedCost.toFixed(4)}`);
        // --- Degradation Notice ---
