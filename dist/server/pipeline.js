 ...
 */
export async function runFusionPipeline(client, sessionID, prompt, config, originalModel, recursionGuard) {
    // -----------------------------------------------------------------------
    ...
    try {
        ...
        const panelResults = await fanOut(client, sessionID, prompt, config.panel.models, config, {
        ...
        for (const r of panelResults) {
            if (!r.error) {
                costTracker.trackPanelist(r.modelId, r.tokenCount.prompt, r.tokenCount.completion);
            }
        }
        // Split results
        ...
        if (successful.length === 0) {
            ...
            const failedModels = failed.map((r) => ({
                modelId: r.modelId,
                reason: r.error,
            }));
        ...
        const judgeOutput = await runJudge(client, sessionID, panelResults, config);
        ...
        if (judgeOutput === null) {
            recursionGuard.markFusionComplete(sessionID);
            const failedModels = failed.length > 0
        ...
        const synthesizedAnswer = await synthesize(client, sessionID, judgeOutput, panelResults, config, originalModel);
