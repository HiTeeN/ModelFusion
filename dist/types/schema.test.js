...
function typeMatchesSchema(value, schemaDef) {
    const expectedType = schemaDef.type;
    if (expectedType === "array") {
        if (!Array.isArray(value))
            return false;
        if (schemaDef.items && value.length > 0) {
            return objectMatchesItemsSchema(value[0], schemaDef.items);
        }
        return true;
    }
...
function objectMatchesItemsSchema(obj, itemSchema) {
    ...
    for (const field of required) {
        if (!(field in obj))
            return false;
        if (!typeMatchesSchema(obj[field], props[field]))
            return false;
    }
...
function validateAgainstSchema(obj, schema) {
    ...
    for (const [field, value] of Object.entries(record)) {
        ...
        if (propDef) {
            if (!typeMatchesSchema(value, propDef)) {
                errors.push(`Field "${field}" has wrong type. Expected "${propDef.type}", got ${Array.isArray(value) ? "array" : typeof value}`);
            }
        }
    }
...
describe("JUDGE_OUTPUT_SCHEMA", () => {
    ...
    test("GIVEN a JudgeOutput with a wrong type for scoring WHEN validated THEN it detects the type mismatch", () => {
        ...
        // THEN
        expect(result.valid).toBe(false);
        expect(result.errors).toContain('Field "scoring" has wrong type. Expected "array", got string');
        expect(result.errors.length).toBe(1);
    });
